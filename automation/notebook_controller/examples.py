...
Run with: python automation/notebook_controller/examples.py
"""
...
NOTEBOOK_PATH = str(PROJECT_ROOT / 'notebooks' / 'Ames_Housing_Price_Prediction_EXECUTED.ipynb')
...
def example_3_cell_manipulation():
    ...
    test_path = str(PROJECT_ROOT / 'notebooks' / 'test_manipulation.ipynb')
...
def example_6_output_management():
    ...
    nb = NotebookController(NOTEBOOK_PATH)
    test_path = str(PROJECT_ROOT / 'notebooks' / 'test_outputs.ipynb')
    nb.save_as(test_path)
...
def example_7_export_import():
    ...
    export_file = str(PROJECT_ROOT / 'notebooks' / 'exported_cell.py')
    exported_content = nb.export_cell(index=7, output_file=export_file)
